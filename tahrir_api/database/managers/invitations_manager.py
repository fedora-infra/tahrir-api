...
# Description: Invitation management module for Tahrir database
...
class InvitationManager(BaseManager):
    """Handles invitation-related database operations"""
    ...
    def add_invitation(self, badge_id, created_on=None, expires_on=None, created_by_email=None):
        ...
        if not self.badges.badge_exists(badge_id):
        ...
        if not created_by_email or not self.persons.person_exists(email=created_by_email):
            raise ValueError(f"No user with email {created_by_email!r}. Ask them to login first.")
        created_by = self.persons.get_person(created_by_email).id
